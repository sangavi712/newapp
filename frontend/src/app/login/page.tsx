...
import { Mail, Lock, Eye, EyeOff, ArrowRight, Leaf, Sun, Moon } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify.error(err.response?.data?.message || 'Invalid email or password. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-12 sm:px-6 lg:px-8 bg-emerald-50/50 dark:bg-[#041E14] transition-colors duration-300">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        ...
        {/* Dotted pattern overlay (like the map in the image) */}
      ...
      >
        <div className="rounded-[2.5rem] border border-white/60 dark:border-emerald-800/40 bg-white/70 dark:bg-[#082417]/80 p-10 shadow-[0_20px_50px_rgba(16,185,129,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-2xl relative overflow-hidden transition-colors duration-300">
          ...
          <div className="mt-8 text-center text-xs">
            <span className="text-slate-500 dark:text-emerald-100/50 transition-colors duration-300">Don't have an account? </span>
            <Link href="/register" className="font-semibold text-emerald-600 dark:text-[#10B981] hover:text-emerald-500 dark:hover:text-[#34D399] transition-colors">
              Sign up
            </Link>
          </div>
        </div>
